...
function buildSyntheticSiz(opts: {
  ...
  v.setUint32(30, 0, false);
  v.setUint32(34, 0, false); // XOsiz/YOsiz
  v.setUint32(38, opts.XTsiz, false);
  v.setUint32(42, opts.YTsiz, false);
  v.setUint32(46, 0, false);
  v.setUint32(50, 0, false); // XTOsiz/YTOsiz
...
describe("extractSizInfo (synthetic)", () => {
  ...
  it("throws ParseError when SIZ is absent", () => {
    expect(() =>
      extractSizInfo(new Uint8Array([0xaa, 0xbb, 0xcc, 0xdd])),
    ).toThrow(ParseError);
  });
  it("throws ParseError when SIZ is truncated", () => {
    expect(() =>
      extractSizInfo(new Uint8Array([0xff, 0x51, 0x00, 0x05])),
    ).toThrow(ParseError);
  });
...
describe.runIf(existsSync(TCI))("extractSizInfo (real TCI 10m)", () => {
  it("returns 10980x10980, 1024x1024 tiles, 3 components", () => {
    ...
    const header = new Uint8Array(
      data.buffer,
      data.byteOffset,
      Math.min(data.byteLength, 100_000),
    );
